...
class CamVidDataset(Dataset):
    def __init__(self, root, images_dir, labels_dir, class_dict_path, resolution, crop=False):
        ...
        # where to crop, only during training not during testing mode!!
        self.crop = crop 
    ...
    def parse_class_dict(self, class_dict_path):
        ...
        class_data = pd.read_csv(class_dict_path)
        class_dict = dict()
        
        for i in range(class_data.shape[0]):
            row = class_data.iloc[i, :]
            class_dict[i] = ((row['r'], row['g'], row['b']), row['name'])
    ...
    def rgb_to_class_id(self, label_img):
        ...
        rgb_array = np.array(label_img)

        class_id_array = np.zeros((rgb_array.shape[0], rgb_array.shape[1]), dtype=np.uint8)
        for class_id, (rgb_value, class_name) in self.class_dict.items():
            mask = np.all(rgb_array == rgb_value, axis=-1)
            class_id_array[mask] = class_id
        
        # np array to Image
        class_id_image = Image.fromarray(class_id_array)
