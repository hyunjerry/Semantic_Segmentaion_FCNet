...
def eval_model(model, dataloader, device, save_pred=False):
    ...
    with torch.no_grad():
        ...
        for images, labels in dataloader:
            ...
            time2 = time.time()
            print(f"first for loop: {time2 - time1} seconds")
            time1 = time.time()
            ...
            time2 = time.time()
            print(f"second for loop: {time2 - time1} seconds")


        #### This! Use the math in paper
        pixel_acc = torch.diag(confusion_matrix).sum() / confusion_matrix.sum()
